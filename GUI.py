...
PASSWORD_ICON_PATH = os.path.dirname(os.path.realpath(
    __file__)) + "/images/password_icon.png"  # password icon path
...
def add_new_known_face():
    faces_recognizer.KNOWN_FACES = refresh_database(
        name=NEW_NAME.get().lower())
...
def display_frames_per_second(frame, start_time):
    ...
    cv2.putText(frame, "FPS: {}".format(
        round(1/max(0.0333, END_TIME), 1)), TEXT_POSITION, FONT, TEXT_SIZE, COLOR)
    return frame
...
def update_frame():
    ...
    photo.paste(image)
    # update displayed image after: round(1000/FPS) [in milliseconds]
...
root.title("Face recognition nhom 7")
...
MESSAGE.set("Để bắt đầu nhận diện ,\nnhập 'password'!")
message_label = tk.Label(root, textvariable=MESSAGE,
                         wraplength="5c", bg="white", fg="red")
message_label.place(relx=0.97, rely=0.080, relwidth=0.2,
                    relheight=0.16, anchor="ne")
...
password_entry.place(relx=0.93, rely=0.290, relheight=0.05,
                     relwidth=0.16, anchor="ne")
...
show_password_button = tk.Button(
    root, command=show_password, image=password_icon, border=0, bg="#131113", activebackground="#131113")
show_password_button.place(relx=0.97, rely=0.290,
                           relheight=0.05, relwidth=0.035, anchor="ne")
show_password_button.bind(show_password)

password_button = ttk.Button(
    root, text="Enter password", command=enter_password)
password_button.place(relx=0.97, rely=0.360,
                      relheight=0.05, relwidth=0.2, anchor="ne")
password_button.bind(enter_password)

NEW_NAME = tk.StringVar()
name_entry = ttk.Entry(root, textvariable=NEW_NAME, state="disabled")
name_entry.place(relx=0.97, rely=0.428, relheight=0.05,
                 relwidth=0.2, anchor="ne")
name_entry.bind('<Return>', enter_name)

name_button = ttk.Button(root, text="Enter your name",
                         command=enter_name, state="disabled")
name_button.place(relx=0.97, rely=0.498, relheight=0.05,
                  relwidth=0.2, anchor="ne")
name_button.bind(enter_name)

screenshot_button = ttk.Button(
    root, text="Take a screenshot", command=take_screenshot)
screenshot_button.place(relx=0.97, rely=0.895,
                        relheight=0.05, relwidth=0.2, anchor="ne")
